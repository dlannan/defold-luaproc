...
#define isequal( L, a, b ) \
  lua_compare( L, a, b, LUA_OPEQ )
...
LUALIB_API int luaopen_luaproc( lua_State *L, const char *name );
...
static void luaproc_loadbuffer( lua_State *parent, luaproc *lp,
  ...
  if ( ret != 0 ) {
    ...
    lua_close( lp->lstate );
#if defined DM_PLATFORM_LINUX || DM_PLATFORM_OSX
...
static void luaproc_openlualibs( lua_State *L ) {
  ...
  requiref( L, LUA_LOADLIBNAME, luaopen_package, TRUE );
  requiref( L, LUA_GNAME, luaopen_base, FALSE );
  luaproc_reglualib( L, LUA_IOLIBNAME, luaopen_io );
  luaproc_reglualib( L, LUA_OSLIBNAME, luaopen_os );
  luaproc_reglualib( L, LUA_TABLIBNAME, luaopen_table );
  luaproc_reglualib( L, LUA_STRLIBNAME, luaopen_string );
  luaproc_reglualib( L, LUA_MATHLIBNAME, luaopen_math );
  luaproc_reglualib( L, LUA_DBLIBNAME, luaopen_debug );
...
LUALIB_API int luaopen_luaproc( lua_State *L, const char *name ) {
  ...
  luaL_register(L, name, luaproc_funcs);
